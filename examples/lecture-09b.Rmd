...
popStates <- get_decennial(geography = "state", year = "2010", variable = "P001001")
```
...
```{r census-missouri-county-pop, results = "hide"}
popMOCounties <- get_decennial(geography = "county", year = "2010", state = "MO", variable = "P001001")
```

Here is the syntax to download only the city of St. Louis:

```{r census-stl-tract-pop, results = "hide"}
popSTL <- get_decennial(geography = "county", year = "2010", state = "MO",
                               county = "St. Louis city", variable = "P001001")
```

For the 2010 census, we can also download data the tract and block group levels:

```{r census-stl-subcounty-pop, results = "hide"}
...
```{r census-stl-racetbl, results = "hide"}
...
```{r census-stl-racetbl-wide, results = "hide"}
...
```{r acs-mo-households, results = "hide"}
...
```{r acs-stl-tract-households, results = "hide"}
...
```{r acs-stl-tract-households-wide, results = "hide"}
...
fs::file_copy(here::here("examples", "lecture-09b.nb.html"), 
              here::here("docs", "lecture-09b.nb.html"), 
              overwrite = TRUE)
```
